...
from core.agents.kr36_reader.configuration import Kr36ReaderConfig
from core.agents.kr36_reader.state import (
    Kr36ArticleInfo,
    Kr36ArticleRecommendation,
    Kr36ReaderState,
    Kr36ReadResult,
)
from core.tools.info_collect.kr36_collect import get_kr36_articles
from core.configs.system_prompt import SYSTEM_PROMPT
from core.agents.kr36_reader.prompts import format_article_data, get_article_task_instruction
...
async def _ainvoke_article_read_with_tools(
    ...
    parser = PydanticToolsParser(tools=[Kr36ReadResult], first_tool_only=True)
    result = await (tool_model | parser).ainvoke(messages)
    if result is None:
        raise ValueError("Model did not return a Kr36ReadResult tool call.")
    return result


def _format_recommendations(
    recommendations: list[Kr36ArticleRecommendation], found_articles: list[Kr36ArticleInfo]
) -> str:
...
async def article_read(
    state: Kr36ReaderState, config: RunnableConfig
) -> Command[Literal["__end__"]]:
    configurable = Kr36ReaderConfig.from_runnable_config(config)
    ...
    raw_articles = get_kr36_articles(max_count=configurable.max_read_articles)
    found_articles = [
        Kr36ArticleInfo(
            id=str(i),
            title=item["title"],
...
kr36_reader_graph = StateGraph(Kr36ReaderState)
kr36_reader_graph.add_node("article_read", article_read)
kr36_reader_graph.add_edge(START, "article_read")
kr36_reader_graph.add_edge("article_read", END)

kr36_reader_graph = kr36_reader_graph.compile()


if __name__ == "__main__":
    result = asyncio.run(kr36_reader_graph.ainvoke({}))
    print(result)
